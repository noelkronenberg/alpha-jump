...
public class JumpSturdyGame {
    ...
    void getColorAndPiecesBoardForFen(String fen){
        ...
        for (int i = 0; i < fen.length(); i++) {
            if (Character.isLetter(fen.charAt(i))){
                ...
                i++;    //Damit wir über die figur springen
                boardCounter++;
            }
    ...
    public void getAllMovesForPlayer(String player, String[] colorsBoard, String[] piecesBoard, String enemyPlayer){ //Player soll sagen, ob wir rot oder blau sind (also Rot = R und Blau = B),
        //TODO: IDEE 3 Arrays 1 Pieces, Colors, Stacks
    ...
    boolean isMovePossible(int from, int to, String[] colorsBoard,String[] piecesBoard,boolean isCapture, boolean isMove){          //IDEE: mann könnte funktionsaufrufe verringern, indem man eine weitere boolean isMove als Parameter hinzufügt
        if (isCapture){
        ...
        if (player=="B"){
            if (piece=="P"){
                if (pos%8==6&&pos!=54){                     //Linker Rand
                    if (pos==6){
                        if (isMovePossible(pos,0,colorsBoard,piecesBoard,true,false)){
                            moves+=boardPostions.get(0)+",";
                        }
                        if (isMovePossible(pos,7,colorsBoard,piecesBoard,false,true)){
                            moves+=boardPostions.get(7)+",";
                        }
                    }
                    else {
                        if (isMovePossible(pos,pos-7,colorsBoard,piecesBoard,true,false)){
                            moves+=boardPostions.get(pos-7)+",";
                        }
                        if (isMovePossible(pos,pos-8,colorsBoard,piecesBoard,false,true)){
                            moves+=boardPostions.get(pos-8)+",";
                        }
                        if (isMovePossible(pos,pos+1,colorsBoard,piecesBoard,false,true)){
                    ...
                    if (pos<=12&&pos>=7) {                              //Case: 1 vor der Roten Baseline
                        //Die moves sind in jedem dieser Fälle drin
